...
class ConfigTest extends TestCase
{
    ...
    /**
     * Assert that Basic auth is configured correctly
     *
     * @return void
     * @throws EmptyValueException
     */
    public function testHasBasicAuth(): void
    {
        ...
        $this->assertEquals(
            expected: $_ENV['BASIC_AUTH_USERNAME'],
            actual: Config::$instance->basicAuth->username
        );
        $this->assertEquals(
            expected: $_ENV['BASIC_AUTH_PASSWORD'],
            actual: Config::$instance->basicAuth->password
        );
    }

    /**
     * Assert that Jwt auth is configured correctly
     *
     * @return void
     * @throws EmptyValueException
     */
    public function testHasJwtAuth(): void
    {
        ...
        $this->assertEquals(
            expected: $_ENV['JWT_AUTH_CLIENT_ID'],
            actual: Config::$instance->jwtAuth->clientId
        );
        $this->assertEquals(
            expected: $_ENV['JWT_AUTH_CLIENT_SECRET'],
            actual: Config::$instance->jwtAuth->clientSecret
        );
        $this->assertEquals(
            expected: $_ENV['JWT_AUTH_GRANT_TYPE'],
            actual: Config::$instance->jwtAuth->grantType
        );
        $this->assertEquals(
            expected: $_ENV['JWT_AUTH_SCOPE'],
            actual: Config::$instance->jwtAuth->scope
        );
    }
